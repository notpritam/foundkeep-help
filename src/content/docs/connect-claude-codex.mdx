...
import agentsDark from '../../assets/screens/agents-dark.png';
...
<Aside type="tip" title="Revoke anytime">
You can revoke access at any time at **foundkeep.app → Settings → Agent
connections**.
</Aside>

<Figure
	src={agentsDark}
	alt="FoundKeep's Agent connections screen — a form to name a connection and choose its access, alongside a short setup walkthrough"
	caption="Agent connections: name a connection, choose what it can read or change, and connect any MCP-compatible client."
	wide
/>
...
<Tabs>
<TabItem label="Claude Code" icon="node">
...
<TabItem label="Claude Desktop" icon="laptop">
...
<p>
	<Badge text="read" variant="success" /> <Badge text="search" variant="success" />
	<Badge text="create" variant="success" /> <Badge text="organize" variant="success" />
	<Badge text="cannot delete" variant="caution" />
</p>

Because it works from what *you* chose to keep, its answers are grounded in your
own sources. See [A second brain for agents](/usecase-second-brain-for-agents/).
